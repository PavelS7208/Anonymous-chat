...
func (h *PostHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.sender.Send(r.Context(), req); err != nil {
		...
		h.handleSenderError(w, roomName, err)
		return
	}
...
func (h *PostHandler) handleSenderError(w http.ResponseWriter, roomName string, err error) {
	switch {
	...
	case errors.Is(err, service.ErrSessionNotFound),
		...
		http.Error(w, "403 Forbidden: invalid session", http.StatusForbidden)

	case errors.Is(err, service.ErrRoomNotFound):
		http.Error(w, "404 Not Found: room does not exist", http.StatusNotFound)
	...
	default:
		h.logger.Warn("unknown service send error", "room", roomName, "err", err)
